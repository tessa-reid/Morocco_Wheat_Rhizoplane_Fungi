...
p1 <- ggplot() + 
  geom_point(data=richness.all, aes(x=reorder(Region, mean), y=mean, color=Region), size=4, shape=15) +
  geom_point(data=richness.isl, aes(x=Region, y=mean, color=Region), size=4, shape=5) +
  geom_errorbar(data=richness.all, aes(x=Region, ymin=mean-sd, ymax=mean+sd, color=Region), width=.2) +
  geom_errorbar(data=richness.isl, aes(x=Region, ymin=mean-sd, ymax=mean+sd, color=Region), width=.2) +
  theme_classic()+xlab("Region")+ylab("Observed ASV richness")+scale_color_manual(values=colour) +
...
p2 <- ggplot() + 
  geom_point(data=diversity.all, aes(x=reorder(Region, mean), y=mean, color=Region), size=4, shape=15) +
  geom_point(data=diversity.isl, aes(x=Region, y=mean, color=Region), size=4, shape=5) +
  geom_errorbar(data=diversity.all, aes(x=Region, ymin=mean-sd, ymax=mean+sd, color=Region), width=.2) +
  geom_errorbar(data=diversity.isl, aes(x=Region, ymin=mean-sd, ymax=mean+sd, color=Region), width=.2) +
  theme_classic()+xlab("Region")+ylab("Faith's Phylogenetic Diversity") +
...
library(ggpubr)
Fig.1 <- ggarrange(Fig.1A, Fig.1B, nrow = 1, common.legend = TRUE, legend = "right", labels = c("A", "B"))
Fig.1
